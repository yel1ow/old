...
uses
  //PngImage,
  Jpeg, GLFileMD2, GLFile3DS, VectorGeometry, GLProxyObjects,
  ...
  GLBitmapFont, IdBaseComponent, IdComponent, IdTCPServer;//,GLFileMAX;
...
type
 ...
 type
  ...
  LifeTimeLeft:real;
    ...
    Procedure Anim(n:integer;xx,yy,zz:real);
    Procedure CheckAOE(n:integer;xx,yy,zz:real);
...
//  Skill:tSkills;
  ...
  Clients:integer;
  skillused:array[0..1] of boolean;
  SkillNumber:array[0..1] of integer;
  sx:array[0..1] of real;
  sy:array[0..1] of real;
  sz:array[0..1] of real;
  Chosed:boolean;
  numChose:integer;
  PredHp:array[0..1]of integer;
  HNames:array[1..MaxHeroes]of TGLSpaceText;
  Clipers:array[0..1] of integer;
  soed:TIdPeerThread;
implementation
...
Begin
  Case Self.TipTarget of
    ...
    FrontTrg: Begin
              ...
              End;
    SelfTrg:  Begin anim(0,MyHeroModel^.Position.x,MyHeroModel^.Position.y,MyHeroModel^.Position.z) End;
   end;
end;
...
Begin
   Case Self.TipTarget of
    ...
    ShotTrg:  Begin
                ...
                Anim(0,AllHeroes[Hero].MyPricel^.Position.x-xx,AllHeroes[Hero].MyPricel^.Position.y-yy,AllHeroes[Hero].MyPricel^.Position.z-zz);
                //Anim(x-xx,y-yy,z-zz);
                //Anim(0,MainF.pricel.Position.y-yy,1);
              End;
    AroundTrg:Begin Anim(0,xx,yy,zz) End;
    FrontTrg: Begin Anim(0,xx,yy,zz) End;
    SelfTrg:  Begin End;
   end;
End;
...
Procedure TSkills.CheckAOE(n:integer;xx,yy,zz:real);
var i:integer;
Begin
  If Self.Hero=Heroes[n].MyNumber
  then i:=1-n
  else i:=n;
...
Procedure Tskills.Anim(n:integer;xx,yy,zz:real);
...
Begin
  ...
  1: begin
      ...
      4:begin
           ...
           CheckAOE(n,Me^.Position.x,me^.Position.y,me^.Position.Z);
        end;
      end;
     end;
  2: begin
...
begin
  ...
  begin
    AddTextureMaterial('TexPole','media\trava.jpg');
...
//('Juggler','pacan.3ds');
 ...
 { Heroes[0]:=allHeroes[3];
  ...
  Heroes[0].me^.position.y:=0;
  Heroes[0].me^.position.z:=0;
  Heroes[1]:=allHeroes[1];
  Heroes[1].me^.position.x:=0;
  Heroes[1].me^.position.y:=0;
  Heroes[1].me^.position.z:=0;
...
then begin
  ...
  then Begin
        ...
        If Heroes[0].MyNumber=3
        then i:=1
        else i:=0;
        x:=heroes[i].me^.position.x;
        y:=heroes[i].me^.position.z;
        If (abs((x-x1)/(x2-x1)-(y-y1)/(y2-y1))<0.05)and(((x>x1)and(x<x2))or((x<x1)and(x>x2)))
        then heroes[i].Health:=heroes[i].Health-AllHeroes[3].skill[1].Damage;
  ...
  then Begin
        ...
        If Heroes[0].MyNumber=AllHeroes[3].MyNumber
        then i:=0
        else i:=1;
        AllHeroes[3].Skill[3].CheckAOE(1,Meteorit.Position.x,meteorit.Position.Y,meteorit.Position.z);
  ...
  then Case key of
        ...
        VK_Return:begin
                   ...
                   For i:=1 to MaxSkills do
                    ...
                    For i:=1 to MaxSkills do
                      begin
                        Heroes[0].CoolDownIcons[i]:=TGLHUDSprite.CreateAsChild(MainF.GLScene1.Objects);
                        Heroes[0].CooldownIcons[i].Material.Texture.Image.LoadFromFile('media\cooldown.jpg');
                        Heroes[0].CoolDownIcons[i].Material.Texture.Disabled:=false;
                        Heroes[0].CoolDownIcons[i].Position.X:=heroes[0].skillsprites[i].Position.x;
                        Heroes[0].CoolDownIcons[i].Position.y:=heroes[0].skillsprites[i].Position.y+heroes[0].skillsprites[i].Height/2+10;
                        Heroes[0].CoolDownIcons[i].Height:=20;
                        Heroes[0].CoolDownIcons[i].Width:=0;
                      end;
...
procedure TMainF.ServerConnect(AThread: TIdPeerThread);
var s:string;
    n:integer;
begin
  AThread.Connection.Write(IntToStr(Clients));
  inc(Clients);
  MainF.Caption := 'Klientov: ' + IntToStr(Clients);
  s := AThread.Connection.CurrentReadBuffer;
  n := StrToInt(Copy(s,2,1));
  if clients = 1
  then begin
         AThread.Connection.Write('ok!');
         Clipers[0] := n;
         Heroes[0] := AllHeroes[n];
         soed := AThread;
       end
  else begin
         if Clipers[0] = n
         then AThread.Connection.Write('no!')
         else begin
                soed.Connection.Write(IntToStr(n));
                AThread.Connection.Write('ok' + IntToStr(Clipers[0]));
                Clipers[1] := n;
                Heroes[1] := AllHeroes[n];
                Chosed := true;
                Heroes[0].me^.position.x:=5;
                Heroes[0].me^.position.y:=0;
                Heroes[0].me^.position.z:=0;
...
procedure TMainF.ServerExecute(AThread: TIdPeerThread);
var s:string;
    i,n,sNumber:integer;
    xx,yy,zz:real;
begin
  s := AThread.Connection.CurrentReadBuffer;
  s := copy(s,1,pos('$', s)-1);
  n := StrToInt(Copy(s,1,1));
  if (heroes[n].MoveAble)and(heroes[n].ControlAble)
  then begin
         if s[2] = '1'
         then heroes[n].Move(0,0,1);
         if s[4] = '1'
         then heroes[n].Move(0,0,-1);
         if s[5] = '1'
         then heroes[n].Move(1,0,0);
         if s[3] = '1'
         then heroes[n].Move(-1,0,0);
       end;
  delete(s,1,5);
  heroes[n].Me^.Direction.X := StrToFloat(copy(s,1,pos(' ', s)-1));
  delete(s,1,pos(' ', s));
  heroes[n].Me^.Direction.Y := StrToFloat(copy(s,1,pos(' ', s)-1));
  delete(s,1,pos(' ', s));
  heroes[n].Me^.Direction.Z := StrToFloat(copy(s,1,pos(' ', s)-1));
  delete(s,1,pos(' ', s));
  if s[1] = '1'
  then begin
         s := s + ' ';
         sNumber := StrToInt(copy(s,2,1));
         delete(s,1,2);
         ...
         delete(s,1,pos(' ', s));
         Heroes[n].Skill[sNumber].Anim(n, xx, yy, zz);
         SkillUsed[n] := true;
         SkillNumber[n] := sNumber;
         sx[n] := xx;
         sy[n] := yy;
         sz[n] := zz;
       end;
  s := FloatToStr(Heroes[n].Me^.Position.X) + ' '
     + FloatToStr(Heroes[n].Me^.Position.Y) + ' '
     + FloatToStr(Heroes[n].Me^.Position.Z) + ' ';

  s := s + FloatToStr(Heroes[1-n].Me^.Position.X) + ' '
         + FloatToStr(Heroes[1-n].Me^.Position.Y) + ' '
         + FloatToStr(Heroes[1-n].Me^.Position.Z) + ' ';

  s := s + FloatToStr(Heroes[1-n].Me^.Direction.X) + ' '
         + FloatToStr(Heroes[1-n].Me^.Direction.Y) + ' '
         + FloatToStr(Heroes[1-n].Me^.Direction.Z) + ' ';
  //HP
  s := s + IntToStr(Heroes[n].Health) + ' ' + IntToStr(Heroes[1-n].Health) + ' ';
  //Eff
  for i:=1 to 5 do
  if Heroes[n].Effects[i]>0
  then s := s + '1'
  else s := s + '0';
  for i:=1 to 2 do
  if Heroes[n].Buffs[i]>0
  then s := s + '1'
  else s := s + '0';
  for i:=1 to 5 do
  if Heroes[1-n].Effects[i]>0
  then s := s + '1'
  else s := s + '0';
  for i:=1 to 2 do
  if Heroes[1-n].Buffs[i]>0
  then s := s + '1'
  else s := s + '0';

  if not SkillUsed[1-n]
  then s := s + '0'
  else begin
         SkillUsed[1-n] := false;
         s := s + '1';
         s := s + IntToStr(SkillNumber[1-n]);
         s := s + FloatToStrF(sx[1-n],ffFixed,1,2) + ' '
                + FloatToStrF(sy[1-n],ffFixed,1,2) + ' '
                + FloatToStrF(sz[1-n],ffFixed,1,2) + ' ';
       end;
